...
class Database {
    ...
    public function exec()
    {
        ...
        try {
            ...
            error_log("SQL Error [500]: " . $e->getMessage() . " | Query: " . $this->getSQL);
            $error = IS_DEV ? $e->getMessage() : 'An unexpected error occurred. Please try again later.';
    ...
    /**
     * Sum a numeric column using the current table and where clauses.
     *
     * @param string $column
     * @return float
     */
    public function sum($column)
    {
        ...
        $sql = "SELECT COALESCE(SUM({$column}), 0) AS total FROM {$this->table}" . $this->where;
        $stmt = $this->raw($sql, $this->bindValues);
        $result = $stmt->fetch();
    ...
    public function where($where, $op = null, $val = null, $type = '', $andOr = 'AND')
    {
        if (is_array($where) && ! empty($where)) {
            ...
            // Backward compatibility: allow calls like where('created_at <=', $value).
            if ($op !== null && $val === null && preg_match('/^([a-zA-Z0-9_\.]+)\s*(<=|>=|!=|<>|<|>)$/', $where, $matches)) {
                $where = $matches[1];
                $val = $op;
                $op = $matches[2];
            }

            // Backward compatibility: allow where('column IS NULL') and where('column IS NOT NULL').
            if ($op === null && preg_match('/^([a-zA-Z0-9_\.]+)\s+IS\s+(NOT\s+)?NULL$/i', $where, $matches)) {
                $this->validate_identifier($matches[1]);
                $where = $matches[1] . ' IS ' . (!empty($matches[2]) ? 'NOT ' : '') . 'NULL';
     ...
     *
     * @param  integer $count  Number of rows to return
     * @return object
     */
    public function limit($count)
    {
        ...
        switch ($driver) {
            ...
            case 'sqlite':
                ...
                break;
            case 'sqlsrv':
                // Store count for combination with offset
                $this->limit = ['sqlsrv_count' => $count];
                break;
        }
    ...
    public function offset($offset)
    {
        ...
        switch ($driver) {
            ...
            case 'sqlite':
                ...
                break;
            case 'sqlsrv':
                $count = (is_array($this->limit) && isset($this->limit['sqlsrv_count']))
                    ? $this->limit['sqlsrv_count']
                    : 0;
                $this->limit = " OFFSET $offset ROWS FETCH NEXT $count ROWS ONLY";
                $this->offset = null;
                break;
        }
    ...
    public function pagination($records_per_page, $page)
    {
        ...
        return $this->limit($records_per_page)->offset($offset);
    }
    ...
    private function buildQuery()
    {
        ...
        if ($this->limit !== NULL) {
            if (is_array($this->limit) && isset($this->limit['sqlsrv_count'])) {
                // SQL Server limit without offset
                $this->sql .= " OFFSET 0 ROWS FETCH NEXT " . $this->limit['sqlsrv_count'] . " ROWS ONLY";
            } else {
                $this->sql .= $this->limit;
            }
        }
